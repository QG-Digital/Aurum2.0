...
function updateNavigation(activePage) {
    // Atualiza o rádio do menu desktop (para mover a animação do glider)
    const radioToSelect = document.getElementById('nav-' + activePage);
    if (radioToSelect) {
        radioToSelect.checked = true;
    }
    
    // Mantém a atualização do menu mobile
...
function closeVideoPopup() {
    ...
    if (videoContainer) {
        ...
        // Pausa o vídeo após fechar para economizar recursos
        document.getElementById('introVideo').pause();
    }
}

// Lógica para mostrar o vídeo apenas na primeira vez
document.addEventListener('DOMContentLoaded', function() {
    const videoContainer = document.getElementById('introVideoContainer');
    
    // Verifica se o usuário já viu o vídeo nesta sessão
    const hasSeenVideo = sessionStorage.getItem('hasSeenIntroVideo');

    if (!hasSeenVideo) {
        // Aguarda 2 segundos após o site carregar para mostrar o vídeo
        setTimeout(() => {
            videoContainer.classList.add('show');
            // Tenta dar play (alguns navegadores exigem que esteja mudo)
...
function toggleMute() {
    ...
    if (video.muted) {
        ...
        // Ícone de som mudo
        muteIcon.innerHTML = '<path d="M11 5L6 9H2v6h4l5 4V5zM15.54 8.46a5 5 0 0 1 0 7.07" />';
    }
}
...
function handleFormSubmit(e) {
    ...
    // 2. Pega o texto da opção selecionada no Assunto (em vez do valor técnico)
    const subjectSelect = e.target.querySelector('select[name="subject"]');
    const subjectText = subjectSelect.options[subjectSelect.selectedIndex].text;
    
    // 3. Monta a mensagem formatada para o WhatsApp
    // %0A é o código para quebra de linha
    // *texto* deixa em negrito e _texto_ em itálico no WhatsApp
    const whatsappMessage = 
        `Olá meu nome é *${name}* e vim do site.%0A` +
        `_${email}_%0A` +
        `_Contato:_ ${phone}%0A%0A` +
        `E eu gostaria de *${subjectText}*.%0A%0A` +
        `${message}`;

    // 4. Codifica a mensagem para o formato de URL
    const encodedMessage = encodeURIComponent(whatsappMessage);
    
    // 5. Define o número de telefone (apenas números)
    const phoneNumber = '5543996349824';
    
    // 6. Mostra uma notificação de sucesso no site antes de redirecionar
    showNotification('Redirecionando para o WhatsApp...', 'success');
    
    // 7. Abre o WhatsApp em uma nova aba após um pequeno delay
    setTimeout(() => {
